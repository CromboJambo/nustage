    ...
    fn apply_step(
        ...
        match &step.step_type {
            ...
            StepType::FilterRows(column, condition) => {
                ...
                // This is a simplified version - actual implementation would parse expressions
                let filter_column = column.clone();
                let filter_condition = condition.clone();

                // For now, we'll use a simple string comparison
                // In a real implementation, this would use expression parsing
                df.filter(col(&filter_column).gt_equal(lit(0)))
                    .map_err(|e| TransformationError::DataError(e.to_string()))
            }
                        ...
                        .agg(
                            aggregations
                                ...
                                .map(|agg| match agg.operation {
                                    ...
                                    AggregationOperation::StdDev => col(&agg.column).std(),
                                    AggregationOperation::Variance => col(&agg.column).var(),
                                })
                                .collect::<Vec<_>>(),
                        )
                        .map_err(|e| TransformationError::DataError(e.to_string()))?;
            ...
            StepType::SortBy(columns, descending) => {
                ...
                let sort_descending: Vec<bool> = vec![*descending; columns.len()];

                df.sort(
                    &sort_columns,
                    SortMultipleOptions::default().with_order(sort_descending),
                )
                .map_err(|e| TransformationError::DataError(e.to_string()))
            }
            ...
            StepType::CustomSql(sql) => {
                ...
                // In a real implementation, this would use DuckDB
                Ok(df.clone())
            }
            StepType::AddColumn(column_name, formula) => {
...
impl TransformationFactory {
    ...
    pub fn select_columns(name: String, columns: Vec<String>) -> TransformationStep {
        TransformationStep {
            id: uuid::Uuid::new_v4().to_string(),
            name,
            step_type: StepType::SelectColumns(columns),
            parameters: HashMap::new(),
            output_schema: Vec::new(), // Will be filled during validation
        }
    }

    /// Create a filter rows transformation
    pub fn filter_rows(name: String, column: String, condition: String) -> TransformationStep {
        TransformationStep {
            id: uuid::Uuid::new_v4().to_string(),
            name,
    ...
    pub fn group_by(
        ...
        TransformationStep {
            id: uuid::Uuid::new_v4().to_string(),
            name,
            step_type: StepType::GroupBy(group_columns, aggregations),
            parameters: HashMap::new(),
            output_schema: Vec::new(), // Will be filled during validation
        }
    }
    ...
    pub fn sort_by(name: String, columns: Vec<String>, descending: bool) -> TransformationStep {
        TransformationStep {
            id: uuid::Uuid::new_v4().to_string(),
            name,
    ...
    pub fn rename_column(name: String, old_name: String, new_name: String) -> TransformationStep {
        TransformationStep {
            id: uuid::Uuid::new_v4().to_string(),
            name,
    ...
    pub fn drop_columns(name: String, columns: Vec<String>) -> TransformationStep {
        TransformationStep {
            id: uuid::Uuid::new_v4().to_string(),
            name,
...
impl fmt::Display for StepType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            StepType::RemoveDuplicates(keep_first) => write!(f, "RemoveDuplicates({})", keep_first),
        }
    }
}
